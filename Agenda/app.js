...
const bigtable = require("@google-cloud/bigtable")({
	projectId: 'pidGoesHere',
	keyFilename: "fileGoesHere",
	zone: 'zoneGoesHere',
	cluster: "clusterGoesHere",
	maxRetries: 5
});
const instance = bigtable.instance("dbGoesHere");
const logTable = instance.table("tableName");
...
const pgdao = new PgDAO(
	{
		user:       process.env.GCP_PGSQL_USERNAME, // default env var: PGUSER
		database:   process.env.GCP_PGSQL_DBNAME,   // default env var: PGDATABASE
		password:   process.env.GCP_PGSQL_PASSWORD, // default env var: PGPASSWORD
		host:       process.env.GCP_PGSQL_HOST,     // Server hosting the postgres database
		port:       process.env.GCP_PGSQL_PORT,     // default env var: PGPORT
 ...
 *    BigTable
 * ================================================================== */
// Init -> Retrieves past log on server start
// logTable.getRows((err, rows) => {
// 	if (err)
// 		console.log(err);
// 	else
// 	{
...
// 		{
// 			logHistory = 	rows[0].data.operations.opcode[0].value + ' -> [' +
// 							rows[0].id+ ']  (' + rows[0].data.operations.duration[0].value + ' ms)\n' +
// 							logHistory;
// 		}
//
...
// });
//
// function bigtablePostLog (opcode, datetime, hrtimeDuration)
// {
// 	hrtimeDuration = process.hrtime(hrtimeDuration);
//
// 	var entries = [
// 		{
// 			key: getLogDateTime(datetime),
// 			data: {
// 				operations: {
// 					opcode: opcode,
// 					duration: hrtimeToMs(hrtimeDuration)
// 				}
// 			}
// 		}
// 	];
//
// 	logTable.insert(entries, (err) => {
// 		if (err)
// 			console.log("Bigtable log failure: " + err);
// 		else {
// 			logHistory = opcode + ' -> [' + entries[0].key + '] (' + entries[0].data.operations.duration + ' ms)\n' + logHistory;
...
function pgCreate (columns)
{
	...
	pgdao.queryCreateUser(columns, (result) => {
		...
		// bigtablePostLog('CAD', datetime, logTimer);
	});
}
...
function pgUpdate (columns)
{
	...
	pgdao.queryUpdateUser(columns.name, columns, (result) => {
		...
		// bigtablePostLog('ALT', datetime, logTimer);
	});
}
...
function pgDelete (request)
{
	...
	pgdao.queryDeleteUser(request.query.name, (result) => {
		...
		// bigtablePostLog('EXC', datetime, logTimer);
	});
}
...
function pgSearchUsers (request)
{
	...
	pgdao.queryRetrieveUsersByParams(params, (result) => {
		...
		// bigtablePostLog('BUS', datetime, logTimer);
	});
}
...
function pgFilterUsers (request)
{
	...
	pgdao.queryRetrieveUsersByParams(params, (result) => {
		sendInfoAndPhotos(result);
		// bigtablePostLog('FILT', datetime, logTimer);
	});
}
...
function pgListUsers (request)
{
	...
	pgdao.queryRetrieveUserList((result) => {
		sendInfoAndPhotos(result);
		// bigtablePostLog('LIST', datetime, logTimer);
	});
}
